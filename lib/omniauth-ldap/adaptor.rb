...
module OmniAuth
  module LDAP
    class Adaptor
      ...
      VALID_ADAPTER_CONFIGURATION_KEYS = [:host, :port, :method, :bind_dn, :password, :try_sasl, :sasl_mechanisms, :uid, :base, :allow_anonymous, :filter, :search_opts]
      ...
      def initialize(configuration={})
        ...
        config[:auth] = @auth
        @search_opts ||= {}
        @search_opts = @search_opts.to_hash
        @search_opts = @search_opts.symbolize_keys if @search_opts.respond_to?(:symbolize_keys)
      ...
      def bind_as(args = {})
        ...
        @connection.open do |me|
          rs = me.search args.merge(@search_opts)
